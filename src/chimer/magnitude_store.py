...
class SiteAmplitudesCollection(BaseModel):
    ...
    def save(self, path: ZipPath) -> None:
        """Save the site amplitudes collection to a file.

        Args:
            path (Path): The path to save the site amplitudes collection to.
        """
        logger.debug("saving site amplitudes to %s", path)
        if not path.is_dir():
            raise ValueError(f"Path {path} is not a directory.")
        meta_file = path / f"{self.source_depth}-{self.quantity}.collection.json"
        with meta_file.open("w") as f:
            f.write(self.model_dump_json(exclude={"site_amplitudes"}))

        pkl_file = path / f"{self.source_depth}-{self.quantity}.collection.pkl"
        with pkl_file.open("wb") as f:
            pickle.dump(self.site_amplitudes, f)

    @classmethod
    def load(cls, path: ZipPath) -> Self:
        """Load the site amplitudes collection from an archive."""
        logger.debug("loading site amplitudes from %s", path)
        pkl_file = path.parent / path.name.replace(".json", ".pkl")
        if not pkl_file.exists():
            raise FileNotFoundError(f"File {pkl_file} not found.")
        with pkl_file.open("rb") as f:
            site_amplitudes = pickle.load(f)

        cls = cls.model_validate_json(path.with_suffix(".json").read_text())
        cls.site_amplitudes = site_amplitudes
        return cls
...
class PeakAmplitudesStore(PeakAmplitudesBase):
    ...
    _loaded_file: Path | None = PrivateAttr(None)
    ...
    def new_collection(self, source_depth: float) -> SiteAmplitudesCollection:
        ...
        collection = SiteAmplitudesCollection(
            ...
            **self.model_dump(exclude={"amplitude_collections"}),
        )
    ...
    def save(self, path: Path | None = None, force: bool = True) -> None:
        ...
        if not path:
            if not self._cache_dir:
                raise EnvironmentError("No cache directory available.")
            path = self._cache_dir

        file = path / f"{self.gf_store_id}-{self.quantity}-{self.hash()}.chz"
        if file.exists() and not force:
            raise FileExistsError(f"File {file} already exists.")

        logger.debug("saving site amplitudes to %s", file)
        with zipfile.ZipFile(file, mode="w") as archive:
            archive_path = zipfile.Path(archive)

            meta_file = archive_path / "meta.json"
            with meta_file.open("w") as f:
                f.write(self.model_dump_json(exclude={"amplitude_collections"}))

            for collection in self.amplitude_collections:
                collection.save(archive_path)

    @classmethod
    def load(cls, file: Path, load_amplitudes: bool = True) -> Self:
        """Load the site amplitudes from a JSON file.

        Args:
            file (Path): The path to the JSON file.
            load_amplitudes (bool, optional): If True, the site amplitudes are loaded.

        Returns:
            PeakAmplitudesStore: The loaded site amplitudes store.
        """
        logger.debug("loading amplitude store from %s", file)
        with zipfile.ZipFile(file, mode="r") as archive:
            archive_path = zipfile.Path(archive)

            meta_file = archive_path / "meta.json"
            amp_store = cls.model_validate_json(meta_file.read_text())

        if load_amplitudes:
            amp_store.load_amplitude_collections(file)

        amp_store._loaded_file = file
        return amp_store

    def load_amplitude_collections(self, archive: Path | None = None):
        """Load the site amplitudes collections from a file.

        Args:
            archive (Path): The path to the file. If None, the file from the last load
                operation is used.
        """
        logger.debug("loading amplitude data from %s", archive)
        archive = archive or self._loaded_file
        if archive is None:
            raise ValueError("No file to load from.")

        with zipfile.ZipFile(archive, mode="r") as archive:
            archive_path = zipfile.Path(archive)
            for archive in archive_path.glob("*.collection.json"):
                collection = SiteAmplitudesCollection.load(archive)
                self.amplitude_collections.append(collection)
...
class PeakAmplitudeStoreCache:
    ...
    def __init__(
        ...
        engine: gf.LocalEngine | None = None,
    ) -> None:
    ...
    def cache_stats(self) -> CacheStats:
        ...
        return CacheStats(
            path=self.cache_dir,
            n_stores=n_stores,
            bytes=ByteSize(nbytes),
        )

    def get_all_cached_stores(self) -> list[PeakAmplitudesStore]:
        stores = []
        for file in self.cache_dir.glob("*.chz"):
            try:
                ...
                continue
            try:
                store = PeakAmplitudesStore.load(file, load_amplitudes=False)
    ...
    def get_cached_stores(
        self, store_id: str, quantity: MeasurementUnit, load_amplitudes: bool = True
        ...
        for file in self.cache_dir.glob("*.chz"):
            ...
            if store_id == store_id and quantity == quantity:
                try:
                    store = PeakAmplitudesStore.load(
                        file, load_amplitudes=load_amplitudes
                    )
                except ValidationError:
                    logger.warning("Invalid store %s, deleting file", file)
                    file.unlink()
